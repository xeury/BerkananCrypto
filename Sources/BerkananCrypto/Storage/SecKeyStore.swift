...
public struct SecKeyStore {
  ...
  public func readOrCreateKey<T: SecKeyConvertible>(label: String) throws -> T {
    if let key: T = try readKey(label: label) {
      return key
    }
    else {
      let key = try T()
  ...
  public func roundTrip<T: SecKeyConvertible>(_ key: T) throws -> T {
